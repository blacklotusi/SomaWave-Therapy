...
@implementation UiSliderSubClass
...
-(id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self) {
        ...
        if ([aDecoder containsValueForKey:@"scrubbingSpeeds"]) {
            self.scrubbingSpeeds = [aDecoder decodeObjectForKey:@"scrubbingSpeeds"];
        } else {
            self.scrubbingSpeeds = [self defaultScrubbingSpeeds];
        }
        
        if ([aDecoder containsValueForKey:@"scrubbingSpeedChangePositions"]) {
            self.scrubbingSpeedChangePositions = [aDecoder decodeObjectForKey:@"scrubbingSpeedChangePositions"];
        } else {
            self.scrubbingSpeedChangePositions = [self defaultScrubbingSpeedChangePositions];
        }
...
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent*)event {
    ...
    bounds = CGRectInset(bounds, -35, -35);
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (_delegate) {
        [_delegate notifyTouchFinish:(UISlider *)self];
        if (currentLocation.y>-25 && currentLocation.y<=60 && currentLocation.x > self.frame.origin.x && currentLocation.x < self.frame.size.width) {
//            [_delegate enableTextEditing:(UISlider *)self];
           NSLog(@"currentLocation x %f currentLocation y %f",currentLocation.x,currentLocation.y);
        }
    }
}
